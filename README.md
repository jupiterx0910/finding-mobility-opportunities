# 🔭 Opportunity Radar

> **Find businesses you are unusually positioned to win — before they become obvious.**

An evidence-driven opportunity discovery system for people moving from **employment to ownership**: B2B salespeople, cross-border operators, industrial technicians, procurement and supply-chain professionals, domain operators using AI, growth operators, creators, merchants, engineers and small teams.

**Latest: v8.0** · [简体中文](README.zh-CN.md)

```text
Human Capital → Cash Flow → Owned Capital
```

Most opportunity analysis asks:

> Which market is growing?

Opportunity Radar asks:

> **What changed? Who is closest to that change? Who will actually pay? Where will the profit pool settle? Can this specific operator enter cheaply, prove recurrence, and convert personal capability into an owned asset?**

---

## Install

```bash
npx skills add https://github.com/jupiterx0910/finding-mobility-opportunities
```

The repository follows the Agent Skills structure: `SKILL.md` contains the operating procedure; deeper logic, cases, scorecards and examples live in `references/`, `cases/` and `examples/`.

---

# The 30-Second Idea

A large industry can be a terrible startup opportunity for you.

A modest market can be life-changing when:

- the underlying need is durable;
- a structural change opens a new entry window;
- your existing skill, customer access, supplier access, geography or tacit knowledge is suddenly repriced upward;
- a clear payer already has budget;
- the first commercial test is cheap and reversible;
- repeated work can become systems, data, software, product, brand, channel, network or equity.

So the real unit of analysis is:

```text
Person × Change × Evidence × Payer × Power × Ownership × Timing
```

---

# The Full Reasoning Chain

```text
1. Structural Need
        ↓
2. Opening Change / Power Redistribution
        ↓
3. Career-to-Founder Transition Fit
        ↓
4. Early Signals: Youth / Campus / Talent / Buyer Behavior
        ↓
5. Real Customer Payment
        ↓
6. Capital + Supply Chain + Formalization
        ↓
7. Public-Market / Economic-Quality Gate
        ↓
8. Power Pool: Who Captures the Surplus?
        ↓
9. Operator-Specific Fit
        ↓
10. 30–60 Day Paid Pilot
        ↓
11. Recurrence
        ↓
12. Assetization
        ↓
13. Real Option: Add / Hold / Kill
```

The order matters because each step eliminates a different false positive.

| Step | Plain-language question | Why it exists |
|---|---|---|
| Structural Need | If today's technology disappeared, would people still want the outcome? | Removes technology looking for a problem. |
| Opening Change | What became cheaper, easier, legal, distributable or newly scarce? | A durable need is not automatically a new opportunity. |
| Career-to-Founder Fit | Which jobs already accumulate customers, transactions, suppliers, tacit knowledge or distribution that can travel with the person? | Some careers are much closer to first revenue than others. |
| Early Signals | Where are behavior, hiring budgets and supplier activity moving? | Financial statements are late. Opportunity discovery needs earlier evidence. |
| Payment | Who pays from which budget, and for what measurable result? | Attention, downloads and funding can exist without a business. |
| Industry Formation | Are roles, suppliers, standards, service firms and infrastructure forming? | Distinguishes a novelty from an ecosystem. |
| Economic Quality | Does the story survive margin, retention, CAC, cash-flow and concentration scrutiny? | Removes growth that is bought, rented or subsidized. |
| Power Pool | Who can keep the profit after competition arrives? | Value creation and value capture are different. |
| Operator Fit | Why can this person enter better than an average outsider? | A 95/100 industry can still be a 30/100 personal opportunity. |
| Paid Pilot | Can reality be tested in 30–60 days without betting the company? | Replaces conviction with a reversible experiment. |
| Recurrence | Why will the same payer pay again? | One sale is not a durable business. |
| Assetization | What remains valuable if the founder stops working for 30 days? | Separates self-employment from ownership. |
| Real Option | What evidence earns the right to invest more, and what kills the thesis? | Prevents both endless analysis and premature all-in bets. |

Read [The Reasoning Chain](references/reasoning-chain.md) for the full causal explanation.

---

# Career-to-Founder Transition Lens

Opportunity Radar does not assume the best founder is a corporate executive, young technical genius or venture-backed founder.

It asks a more practical question:

> **Which jobs are already quietly accumulating the ingredients of a business?**

A career becomes a strong founder launchpad when it accumulates several of these:

```text
transaction proximity
+ payer access
+ repeated customer pain
+ supplier access
+ tacit knowledge
+ trust / reputation
+ distribution
+ low-cost testability
+ an assetization path
```

High-potential transition archetypes often include:

1. **Industrial B2B sales / channel roles** — close to buyers, budgets, objections and reorder cycles.
2. **Cross-border sales / sourcing / ecommerce operators** — close to products, suppliers, trade workflows and foreign customers.
3. **Industrial service / maintenance / automation technicians** — close to failures, downtime, repair economics and field bottlenecks.
4. **Procurement / supply-chain / quality professionals** — close to supplier maps, price, reliability and productization opportunities.
5. **Domain operators who can use AI** — close to a recurring workflow that new AI capabilities can compress or automate.
6. **Growth / content / commercialization operators** — close to customer acquisition, conversion and distribution.

Read [Career-to-Founder Transition Lens](references/career-to-founder-transition.md).

---

# Why This Goes Beyond a Typical Opportunity Analysis

Opportunity Radar does **not** claim SWOT, PESTEL, Five Forces, Lean Startup, JTBD or Business Model Canvas are wrong. They are useful specialist tools.

The difference is the end-to-end decision chain.

| Framework | Strong at | Gap if used alone |
|---|---|---|
| SWOT | organizing a situation | does not force payer proof, timing, Power Pool, pilot or ownership path |
| PESTEL | macro scanning | macro change may never become an accessible business for this operator |
| Five Forces | industry structure and bargaining power | does not answer why this person can enter now or how to validate cheaply |
| TAM / market sizing | estimating scale | a huge market can remain inaccessible or economically weak |
| Lean Startup | experiment and learning loops | does not by itself choose which structural opening deserves the experiment |
| JTBD | understanding customer progress | does not by itself locate timing, founder access or durable value capture |
| Business Model Canvas | describing a business model | a weak opportunity can still produce a beautiful canvas |
| **Opportunity Radar** | linking **change → evidence → career edge → payer → power → pilot → recurrence → ownership** | intentionally broad; specialist tools remain useful inside each layer |

The claim is therefore not “one framework replaces everything.”

It is:

> **Most frameworks analyze one layer. Opportunity Radar links the layers into a falsifiable ownership decision.**

Read [Framework Comparison](references/framework-comparison.md).

---

# Public Example Policy

All public examples in this repository are **synthetic / fictional operator archetypes**.

They are **not based on any real user, private conversation, private profile or inferred personal history**. Public examples must be generated from public market patterns and generic role archetypes only.

This rule is also enforced in `SKILL.md` and `CONTRIBUTING.md`.

---

# Founder Examples — Same World, Different Entry Wedges

All examples below are synthetic and are not recommendations for a specific real person.

- [B2B Sales → Vertical Distribution](examples/b2b-sales-to-distribution.md)
- [Cross-Border Operator → Managed Trade Infrastructure](examples/cross-border-operator.md)
- [Industrial Technician → Service → Product](examples/industrial-technician-to-business.md)
- [Procurement / Supply Chain → Sourcing Infrastructure](examples/procurement-supply-chain-to-business.md)
- [Domain Operator + AI → Vertical AI Service](examples/domain-operator-plus-ai.md)
- [Growth Operator → Owned Distribution](examples/growth-operator-to-owned-business.md)

Each example must convert:

```text
existing edge
→ named payer
→ concrete offer
→ 30–60 day paid pilot
→ recurrence
→ Power Pool
→ assetization
→ kill criterion
```

---

# Proof, Not Just Framework

Historical backtests ask:

> Using only signals that were observable early, would the framework have asked useful questions, identified an accessible entry window, and avoided obvious false positives?

Initial backtests:

- [E-commerce](cases/ecommerce.md)
- [Mobile App Economy](cases/mobile-app-economy.md)
- [Creator Economy](cases/creator-economy.md)
- [E-cigarettes](cases/e-cigarette.md)
- [AI-Native Services](cases/ai-native-services.md)

Backtests are not proof that the framework predicts winners. They test whether the framework's questions, vetoes and staged-commitment logic are useful before certainty exists.

---

# Monthly Opportunity Radar

The [August 2026 Radar](radar/2026-08.md) is a dated watchlist, not a “hot sector” ranking.

Every entry must include:

- geography and as-of date;
- the structural change;
- payer hypothesis;
- best-fit founder archetypes;
- Power Pool hypothesis;
- leading indicators;
- confirmation indicators;
- kill criteria;
- unknowns and confidence.

The purpose is to create a public decision log that later editions can **confirm, downgrade or kill**.

---

# Two Scores, Not One

### Signal Confirmation Score

> Is the opportunity becoming economically real?

Looks at payment, retention, hiring, supply-chain formation, capital quality, formalization, diffusion and persistence.

### Mobility Opportunity Score

> Is this a good opportunity for this specific operator?

Looks at founder edge, payer access, affordable entry, time to cash, leverage, assetization, Power Pool access, competition and regulation.

```text
Good industry ≠ Good company ≠ Good investment ≠ Good startup for this person
```

---

# Three Books, Three Failure Modes

Opportunity Radar recommends only three entrepreneurship books in its core reading path:

1. **The Mom Test — Rob Fitzpatrick**: find customer truth instead of collecting polite feedback.
2. **The Lean Startup — Eric Ries**: turn assumptions into cheap, fast learning loops.
3. **7 Powers — Hamilton Helmer**: understand how a working business develops durable economic power.

```text
Customer Truth → Validated Learning → Durable Power
```

Read [Why These Three Books](references/recommended-reading.md).

---

# What the Skill Returns

A substantive run should produce:

1. operator advantage + entrepreneur archetype;
2. career-to-founder transition assessment;
3. structural need + opening change;
4. industry stage + dated evidence;
5. signal radar with **why each signal matters**;
6. 3–7 concrete businesses, not vague sectors;
7. user / beneficiary / buyer / payer architecture;
8. Signal Confirmation Score + Mobility Opportunity Score;
9. Power Pool and surplus-capture analysis;
10. unit economics + first-sale plan;
11. recurrence mechanism;
12. assetization path;
13. leading / confirmation / kill criteria;
14. weakest-link analysis;
15. final verdict: **START / BUY A REAL OPTION / WATCH / REJECT**.

---

# Repository Structure

```text
.
├── README.md
├── README.zh-CN.md
├── SKILL.md
├── CHANGELOG.md
├── CONTRIBUTING.md
├── references/
├── examples/
├── cases/
├── radar/
├── scripts/
├── tests/
└── .github/workflows/
```

---

# Design Principles

- **Evidence before narrative.** A good story is not a signal.
- **Payment before popularity.** Users and payers may be different people.
- **Career edge before prestige.** Founder-market fit comes from real access, not title.
- **Power before TAM worship.** Large value pools do not guarantee accessible profit pools.
- **Pilot before product.** Buy information cheaply before committing capital.
- **Recurrence before scale.** One sale is not a business model.
- **Assetization before vanity growth.** The goal is ownership, not merely a busier job.
- **Kill criteria before conviction.** A thesis should be able to die.
- **Backtest before boasting.** Do not rewrite history around winners.
- **Synthetic public examples only.** Never turn private user context into public repository content.

---

## One Sentence

> **Opportunity Radar finds structural change, identifies the careers and operators closest to it, verifies real payment and value capture, and converts the thesis into the cheapest paid test that can eventually become owned capital.**
